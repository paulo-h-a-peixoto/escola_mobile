...
export default () => {
    ...
    const handleSaveMedico = async () => {
        if(nome !== '' && telefone !== '' && endereco !== '' && crm !== '' && especialidade != '' && data !== '' && hora !== '' && select !== 'Selecione uma opção') {
            if(select == 'outro'){
                var result = await api.addMedico(nome, telefone, endereco, crm, especialidade, data, hora, outro);
            }else{
                var result = await api.addMedico(nome, telefone, endereco, crm, especialidade, data, hora, select);
            }
